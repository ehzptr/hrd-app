...
def _checkbox(state: FletDashboardState, key: str, label: str, value: bool) -> ft.Checkbox:
    control = ft.Checkbox(label=label, value=value, dense=True)
            ...
            controls=[
                ft.Row(
                    controls=[
                        ...
                        ft.Text("WAJIB" if required else "OPSIONAL", size=9, color=TEAL if required else SLATE_500),
                    ]
                ),
...
def _template_button(label: str, icon: str, data_factory: Callable[[], bytes], state: FletDashboardState) -> ft.Button:
    ...
    async def save(_: ft.ControlEvent) -> None:
        path = await picker.save_file(file_name=label, allowed_extensions=["xlsx"])
        if path:
...
def build_sidebar(state: FletDashboardState) -> ft.Container:
    """Build the complete Flet sidebar migrated from the NiceGUI application.

    All controls are registered in ``state.config_fields`` so the state layer
    can build one validated pipeline request when the user clicks Process.
    """
    ...
    config_section = ft.Column(controls=[schedule, ft.Divider(), scoring, ft.Divider(), sales], spacing=10)
    ...
    return ft.Container(
        ...
        content=ft.Column(
            controls=[
                ft.Text("HR Dashboard", size=24, weight=ft.FontWeight.BOLD, color=NAVY),
                ft.Text("Attendance · Payroll · Sales Performance", size=12, color=SLATE_500),
                ft.Divider(),
                upload_section,
                ft.Divider(),
                config_section,
                ft.Divider(),
                _section_title("3", "Proses Data"),
